...
<html>
	...
	<?php
		...
		if ($json === false) {
			die('Error reading the JSON file: '.dirname(__FILE__).'/config.json');
		}
		if(is_writable($cfg)) {		
	?>	
		<script>
	...
	?>
		<script>
			...
			$("#youtube-dl_err").hide();
/*			$("#youtube-dl_check").remove();;
			$("#youtube-dl_text").remove();;
			$("#youtube-dl_check_text").remove();		
			$("#ytdl").find("br").remove(); */
		</script>
	<?php
		}
