...
4. Songlyrics  
	at moment there are 2 methods to get the songlyrics: genius.com and local files  
 	generally any script that takes "artist=xxx" and "title=xxx" as parameters and outputs (html-)formatted text could be used.  
    
 	For local files the extensions ".txt" and ".lirc" are supported   
	The files need to be located in the songfolder and must contain the songtitle	  
	The ".lirc" files are generated by [lrcget](https://github.com/tranxuanthang/lrcget/tree/main) and may contain the songposition which will be filtered for plain display  
	If a file is found it will be displayed instead of querying genius.  
	...
	ToDo: enable Songlyrics in the configuration page for local use only without genius lyrics	 
